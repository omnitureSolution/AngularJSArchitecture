...
; (function ($) {
    $.fn.scriptcam = function (options) {
        ...
        return this.each(function () {
            ...
            if (fileExists(decodeURIComponent("scripts/" + data.path) + 'scriptcam.swf')) {
                swfobject.embedSWF(decodeURIComponent("scripts/" + data.path) + 'scriptcam.swf', opts.id, newWidth, newHeight, '11.6', false, opts, params);
            }
            else {
                alert(decodeURIComponent("scripts/" + data.path) + 'scriptcam.swf not found, please check the path parameter');
            }
        });
    };
