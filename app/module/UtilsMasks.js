...
(function (angular) {
    ...
    (function () {
        ...
        function convertDate(_DateToModify, inputFormat) {
            function pad(s) { return (s < 10) ? '0' + s : s; }
            var d = new Date(_DateToModify, inputFormat);
        ...
        .directive('xgenTime', ['$log', function ($log) {
            ...
            return {
                ...
                link: function (scope, element, attrs, ctrl) {
                    ...
                    function validateTime(s) {
                        if (s.length == 8) {
                            var t = s.split(':');
                            return /^\d\d:\d\d:\d\d$/.test(s) && t[0] >= 0 && t[0] < 24 && t[1] >= 0 && t[1] < 60 && t[2] >= 0 && t[2] < 60;
                        }
                        else if (s.length == 5 && angular.isDefined(attrs.uiTimeMask) && attrs.uiTimeMask === 'short') {
                            var t = s.split(':');
                            return /^\d\d:\d\d$/.test(s) && t[0] >= 0 && t[0] < 24 && t[1] >= 0 && t[1] < 60;
                        }
                        else {
                            return true;
                        }
                    ...
                    function formatter(value) {
                        ...
                        if (validateTime(formattedValue.replace(/[^0-9]$/, ''))) {
                            return formattedValue.replace(/[^0-9]$/, '');
                        } else {
                            if (formattedValue.length == 5 && angular.isDefined(attrs.uiTimeMask) && attrs.uiTimeMask === 'short')
                                return "00:00";
                            else
                                return "00:00:00";
                        }
        ...
        .directive('xgenDate', ['$locale', '$log', '$injector', '$filter', function ($locale, $log, $injector, $filter) {
            return {
                restrict: 'A',
                require: 'ngModel',
                link: function (scope, element, attrs, ctrl) {
                    element.bind('blur', function () {
                        ...
                        if (!angular.isUndefined(attrs.xgensetonblur) && attrs.xgensetonblur == "true" && (angular.isUndefined(ctrl.$viewValue) || ctrl.$viewValue == '')) {
                            if (angular.isUndefined(ctrl.$viewValue) || ctrl.$viewValue == '') {
                            ...
                            if (angular.isDefined(attrs.xgenDate) && attrs.xgenDate === 'short') {
                                unformattedValueLength = 4;
                                formattedValueLength = 5;
                                timeFormat = '00:00';
                            }
                            ...
                            if (kendo.parseDate(ctrl.$viewValue, attrs.kFormat.replace(/["']/g, "")) == null && !angular.isUndefined(ctrl.$viewValue)) {

                                var _Month = ctrl.$viewValue.substring(0, 2);
                                var _Day = ctrl.$viewValue.substring(2, 4);
                                var _Year = ctrl.$viewValue.substring(4, 8);

                                var _FinalDate = _Month + '/' + _Day + '/' + _Year;

                                if (kendo.parseDate(_FinalDate, attrs.kFormat.replace(/["']/g, "")) != null)
                                    ctrl.$setViewValue(new Date(_FinalDate));
        ...
        .directive('xgenDateTime', ['$locale', '$log', '$injector', '$filter', function ($locale, $log, $injector, $filter) {
            return {
                restrict: 'A',
                require: 'ngModel',
                link: function (scope, element, attrs, ctrl) {
                    element.bind('blur', function () {
                        ...
                        if (!angular.isUndefined(attrs.xgensetonblur) && attrs.xgensetonblur == "true" && (angular.isUndefined(ctrl.$viewValue) || ctrl.$viewValue == '' || ctrl.$viewValue == null)) {
                            if (angular.isUndefined(ctrl.$viewValue) || ctrl.$viewValue == '' || ctrl.$viewValue == null) {
                                //ctrl.$setViewValue(new Date(attrs.kFormat));
                                ctrl.$setViewValue(dateFormat(new Date(), "mm/dd/yyyy HH:MM:ss"));
                                ctrl.$render();
                            }
                        }
                        else {
                            ...
                            if (kendo.parseDate(ctrl.$viewValue, attrs.kFormat.replace(/["']/g, "")) == null && !angular.isUndefined(ctrl.$viewValue)) {
                                ...
                                var ctldate = new Date(ctrl.$viewValue);
                                if (ctldate.toString() != "NaN" && ctldate.toString() != "Invalid Date") {
                                    return;
                                }

                                ctldate = ctrl.$viewValue.replace('/', '');
                                ctldate = ctldate.replace('/', '');
                                ctldate = ctldate.replace('\\', '');
                                ctldate = ctldate.replace('-', '');
                                ctldate = ctldate.replace('-', '');
                                ctldate = ctldate.replace(' ', '')
                                ...
                                var _Month = ctldate.substring(0, 2);
                                var _Day = ctldate.substring(2, 4);
                                var _Year = ctldate.substring(4, 8);
                                var _Hour = ctldate.substring(8, 10);
                                var _Min = ctldate.substring(10, 12);
                                var _Sec = ctldate.substring(12, 14);

                                if (_Year.length == 2)
                                    _Year = "20" + _Year;

                                if ((angular.isUndefined(_Hour)) || _Hour.length == 0)
                                    _Hour = "00";
                                if ((angular.isUndefined(_Min)) || _Min.length == 0)
                                    _Min = "00";
                                if ((angular.isUndefined(_Sec)) || _Sec.length == 0)
                                    _Sec = "00";

                                var _FinalDate = _Month + '/' + _Day + '/' + _Year + ' ' + _Hour + ':' + _Min + ':' + _Sec;


                                var ctldate = new Date(_FinalDate);
                                if (ctldate.toString() != "NaN" && ctldate.toString() != "Invalid Date") {
                                    ctrl.$setViewValue(new Date(_FinalDate));
                                }
                                else {
        ...
        .directive('xgenFormatDateTime', ['$locale', '$log', '$injector', '$filter', function ($locale, $log, $injector, $filter) {
            return {
                restrict: 'EA',
                require: 'ngModel',
                link: function (scope, element, attrs, ctrl) {
                    ...
                    scope.$watch(attrs.ngModel, function () {
                        ...
                        if (!angular.isUndefined(ctrl.$viewValue) && ctrl.$viewValue != "" && IsChanged=="N") {
                            var ctldate = ctrl.$viewValue;
                            //if (kendo.parseDate(ctrl.$viewValue, attrs.kFormat.replace(/["']/g, "")) != null) {
                            //    return;
                            //}
                            ...
                            var _Month = ctldate.substring(4, 6);
                            var _Day = ctldate.substring(6, 8);
                            var _Year = ctldate.substring(0, 4);
                            var _Hour = ctldate.substring(9, 11);
                            var _Min = ctldate.substring(11, 13);
                            var _Sec = ctldate.substring(13, 15);
                            var _MillSec = ctldate.substring(16, 19);
        ...
        .directive('uiInputRequired', ['$locale', '$log', '$injector', function ($locale, $log, $injector) {
            return {
                restrict: 'A',
                require: 'ngModel',
                link: function (scope, element, attrs, ctrl) {
                    element.addClass('validbackcolor');
                    element.bind('blur', function () {
                        if (!angular.isUndefined(ctrl.$viewValue) && ctrl.$viewValue != '')
                            element.removeClass('validbackcolor');
                        else
                            element.addClass('validbackcolor');
        ...
        .directive('xgenToolTip', ['$window', '$compile', 'cssInjector', function ($window, $compile, cssInjector) {

            cssInjector.add("Styles/angular-tooltips.css");
        ...
        .directive('xgenLoader', ['$compile', function ($compile) {
            return {
                restrict: 'A',
                scope: false,
                link: function (scope, element, attrs) {
                    ...
                    scope.$watch(attrs.xgenLoader, function (loading) {
                        if (loading === true) {
                            
                            $('.LawXmobileloading').show();
                        } else {
                            // HERE I WANT TO RESTORE THE ORIGINAL CONTENT
                            $('.LawXmobileloading').hide();
                        }
                    });
                }
            };
