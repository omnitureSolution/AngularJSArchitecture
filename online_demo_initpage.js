var _dwtParam = {
    ...
    'productKey': '231E0995E29A8EA2BF6AC36A18D04863A36EDE7881E7BB62CBA6AED73559B13310000000',


    'containerID': 'dwtcontrolContainer',   //The ID of Dynamic Web TWAIN control div in HTML.This value is required.

    'width': 450,        //The width of Dynamic Web TWAIN control. This value is optional. The default value is '580'.
    'height': 600,       //The height of  Dynamic Web TWAIN control.This value is optional. The default value is '600'.
    'isTrial': 'false',
    'version': '10,1',
...
var _strDefaultSaveImageName = "ScanDocument";
...
//Sample
//var _strActionPage = "Scan/SaveToFile.aspx";
var scan = document.getElementById("txtScan").value;

var _strActionPage = "SaveToFile.aspx?Scan=" + scan;
...
function pageonload() {
    ...
    initMessageBox(true);  //Messagebox
...
function initCustomScan() {
    var ObjString = "<div id='divTwainType' > ";
    ObjString += "<div  class='col-md-12 col-lg-12 col-sm-12 col-xs-12'>";
    ...
    ObjString += "<label for = 'Duplex'><input type='checkbox' id='Duplex'/>Duplex</label></div>";
    ObjString += "<div  class='col-md-12 col-lg-12 col-sm-12 col-xs-12'>";
    ...
    ObjString += "<label for='RGB'><input type='radio' id='RGB' name='PixelType'/>Color</label></div>";
    ObjString += "<div  class='col-md-12 col-lg-12 col-sm-12 col-xs-12'>";
    ObjString += "<label for='Resolution'>Resolution:<select size='1' id='Resolution'><option value = ''></option></select></label></div>";
    ObjString += "</div>";
...
function initMessageBox(bNeebBack) {
    ...
    objString += "<div style='text-align:center; width:" + 450 + "px; background-color:#FFFFFF;";
    if (_bShowNavigatorPanel)
        objString += "display:block'>";
    else
        objString += "display:none'>";
    objString += "<div style='display:none;position:relative; background:white; float:left; width:430px; height:35px; z-index:2;'>";
    ...
    objString += "<div style='display:none;position:relative; background:white; float:left; width:150px; height:35px;z-index:2;'>Preview Mode";
    ...
    objString += "<div style='display:none;position:relative;'><input id='DW_btnRemoveCurrentImage' onclick='btnRemoveCurrentImage_onclick()' type='button' value='Remove Selected Images'/>";
    if (bNeebBack) {
        objString += "<input id='DW_btnRemoveAllImages' onclick='btnRemoveAllImages_onclick()' type='button' value='Remove All Images'/><br /><br />";
        objString += "<span style=\"font-size:larger\"><a href =\"online_demo_list.aspx\"><b></b></a></span><br /></div>";
    }
    else
    ...
    objString += "<div id='DWTdivMsg' style='width:" + 450 + "px;";
    if (_bShowMessagePanel)
        objString += "display:inline'>";
    else
        objString += "display:none'>";
    objString += "Message:<br />"
    objString += "<div id='DWTemessage' style='width:" + 450 + "px;height:80px; overflow:scroll; background-color:#ffffff; border:1px #303030; border-style:solid; text-align:left; position:relative' >";
    objString += "</div></div>";
...
function setDefaultValue() {
    ...
    // document.getElementById("MultiPagePDF").checked = true;
...
function controlDetect() {
    ...
    if (DWObject) {
        if (DWObject.ErrorCode == 0) {
            clearInterval(seed);
            DWObject.MaxImagesInBuffer = 2560000;     // This is default value. User can change this value later
            DWObject.loglevel = 1;
            ...
            if (!_bInIE) {
                for (var i = 0; i < navigator.mimeTypes.length; i++) {
                    if (navigator.mimeTypes[i].type.toLowerCase().indexOf("Application/DynamicWebTwain-Plugin".toLowerCase()) > -1) {
...
function createNonAllowedDivPlugin() {
    ...
    ObjString += "<ul>";
    ObjString += "<li>Please click \"Always run on this site\" for the prompt \"" + _strProductName + " Plugin needs your permission to run\", refresh/restart the browser and try again.</li>";
    ObjString += "</ul>";
    ObjString += "</div>";
